...
POI_INSERT_PREFIX = (
    ...
    '(overture_id, name, name_local, name_en, name_local_lang, dedupe_name, lat, lng, geohash, primary_poi_type, brand, '
...
def run_country(batch, country_source_r2_key):
    ...
    rows = paged(
        'overture_candidate',
        ('overture_id', 'name', 'name_local', 'name_en', 'name_local_lang', 'lat', 'lng', 'address', 'category',
...
def run_country_overrides(country_source_r2_key, batch=None):
    ...
    for start in range(0, len(ids), MAX_VALUES_TERMS):
        ...
        rows = d1_client.select(
            'SELECT overture_id, name, name_local, name_en, name_local_lang, lat, lng, address, category, category_path, '
